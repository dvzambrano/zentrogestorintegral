        ...
        init : function(App) {
            ...
            this.entityCycleButton = new Ext.CycleButton({
                ...
                changeHandler:function(btn, item){
                    ...
                    var button = Ext.getCmp('principal-menu-item-home');
                    if(getModule(App.centerPanel.getLayout().activeItem.id.replace('gridPanel', '')).is_multientidable){
                        ...
                        if(button && App.entityCycleButton.getActiveItem().text != bundle.getMsg('entity.field.none'))
                            button.fireEvent('click', button);
                    }
                }
            });
            ...
            this.mainmenuTreePanel = new Ext.tree.TreePanel({
                ...
                loader: new Ext.tree.TreeLoader({
                    ...
                    listeners: {
                        ...
                        load: function(treeLoader, node, response){
                            ...
                            var getMenuItems = function(data, showtext){
                                var items = new Array();
                                for (var i = 0; i < data.length; i++) {
                                    ...
                                    if(data[i].is_active){
                                        ...
                                        if (data[i].children && data[i].children.length > 0) {
                                            ...
                                            item.menu.add(getMenuItems(data[i].children, true));
                ...
                listeners: {
                    click: function(node, eventObject){
                        var button = Ext.getCmp('principal-menu-item-'+node.id);
                        if(button)
                            button.fireEvent('click', button);
                    }
                }
